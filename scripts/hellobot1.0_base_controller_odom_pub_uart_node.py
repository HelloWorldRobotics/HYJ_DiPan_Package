...
class Hellobot1_Base_Driver:
    ...
    const.Wheelbase=0.348                                                      #Wheel base default 0.348
    const.EncoderResolution=1200                                               #Encoder ticks per revolution
    const.WheelDiameter=0.129                                                  #Wheel diameter org value = 0.129
    ...
    def run(self):  
        ...
        rospy.init_node('Hellobot1_Odometry_Publisher', anonymous=True) 
        rospy.Subscriber("/planner/cmd_vel", Twist, self.odom_analysis)
        rospy.loginfo("HelloBot1.0 Connected Succesfully!")

        self.odomPub = rospy.Publisher('/vehicle/odom', Odometry, queue_size=100)
        ...
        delay = rospy.Rate(100.0)

        while not rospy.is_shutdown(): 
            #Receiving wheel direction and speed from STM32 and update odometry
            while self.ser.inWaiting() > 0:
                Hellobot1_Base_Driver.DataInput=self.ser.read(6)
                #rospy.loginfo("Hellobot1_Base_Driver.DataInput[0]=%s",ord(Hellobot1_Base_Driver.DataInput[0]))
                #if ord(Hellobot1_Base_Driver.DataInput[0]) == 2:
                (Hellobot1_Base_Driver.vx,Hellobot1_Base_Driver.vy,Hellobot1_Base_Driver.vth)=\
                self.odom_synthesis(ord(Hellobot1_Base_Driver.DataInput[3]),ord(Hellobot1_Base_Driver.DataInput[5])\
                                       ,ord(Hellobot1_Base_Driver.DataInput[0]),ord(Hellobot1_Base_Driver.DataInput[2]))
                    
                current_time = rospy.Time.now()
                dt = (current_time - previous_time).to_sec()
                #rospy.loginfo("dt=:%f", dt)
                self.delta_x = (Hellobot1_Base_Driver.vx*cos(Hellobot1_Base_Driver.th) - Hellobot1_Base_Driver.vy*sin(Hellobot1_Base_Driver.th)) * dt 
                self.delta_y = (Hellobot1_Base_Driver.vx*sin(Hellobot1_Base_Driver.th) + Hellobot1_Base_Driver.vy*cos(Hellobot1_Base_Driver.th)) * dt 
                self.delta_th= Hellobot1_Base_Driver.vth * dt
                ...
                #rospy.loginfo("Left_Wheel_Actual_Direction=:%s", ord(Hellobot1_Base_Driver.DataInput[3]))
                #rospy.loginfo("Left_Wheel_Actual_Speed=:%s", ord(Hellobot1_Base_Driver.DataInput[5]))
                #rospy.loginfo("Right_Wheel_Actual_Direction=:%s", ord(Hellobot1_Base_Driver.DataInput[0]))
                #rospy.loginfo("Right_Wheel_Actual_Speed=:%s", ord(Hellobot1_Base_Driver.DataInput[2]))
